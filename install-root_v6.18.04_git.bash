...
function clone_from_git_and_install {
    git clone https://github.com/root-project/root.git
    mkdir root-build
    mkdir root-install
    cd root
    #git checkout -b remotes/origin/v6-18-00-patches
...
function printHelp {
    ...
    echo " [0] -sd    : Download"
    echo " [0] -clone : clone from git and install"
    echo " [0] -si    : Install"
    echo " [0] -c     : clean"
    echo " [0] -h     : help"
}
...
then    
	...
	time clone_from_git_and_install
    elif [ "$1" = "-si" ]; then
	installRoot
    elif [ "$1" = "-c" ]; then
	cleanAll
